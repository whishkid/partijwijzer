...
<div class="bg-green-200 font-bold p-2 rounded-xl">Vink 2-4 partijen aan die je wilt vergelijken</div>
<div class="grid grid-cols-1 sm:grid-cols-2 gap-x-2 lg:grid-cols-3 p-2">
	{#each parties.sort((a, b) => (b.name < a.name ? 1 : a.name == b.name ? 0 : -1)) as party}
		<div class="flex flex-row whitespace-nowrap">
			<input id={party.id} type="checkbox" bind:group={chosenParties} value={party} class="mr-2" />
			<label class="truncate select-none" title={party.name} for={party.id}>{party.name}</label>
		</div>
	{/each}
</div>
...
<Warning visible={chosenParties.length > maxParties} text="Kies maximaal 8 parijen" />
